...
bitshuffle8_neon(const uint8_t* const src, uint8_t* dest, const size_t nbyte) {
  ...
  for (i = 0, k = 0; i < nbyte; i += 64, k++) {
    ...
    for (j = 0; j < 8; j++) {
      ...
      vst1_lane_u8(dest + k + j*nbyte/(8*elem_size), r0[0].val[0], 0);
      vst1_lane_u8(dest + k + j*nbyte/(8*elem_size) + nbyte/8, r0[0].val[1], 0);
      vst1_lane_u8(dest + k + j*nbyte/(8*elem_size) + nbyte/4, r0[1].val[0], 0);
      vst1_lane_u8(dest + k + j*nbyte/(8*elem_size) + 3*nbyte/8, r0[1].val[1], 0);
      vst1_lane_u8(dest + k + j*nbyte/(8*elem_size) + nbyte/2, r0[2].val[0], 0);
      vst1_lane_u8(dest + k + j*nbyte/(8*elem_size) + 5*nbyte/8, r0[2].val[1], 0);
      vst1_lane_u8(dest + k + j*nbyte/(8*elem_size) + 3*nbyte/4, r0[3].val[0], 0);
...
main()
{
  ...
  size_t size = 512;
  bitshuffle8_neon(src, dest, size);
  printmem16(dest);
  printmem16(dest + 16);
  printmem16(dest + 32);
  printmem16(dest + 48);
