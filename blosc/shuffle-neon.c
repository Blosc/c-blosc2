        ...
        for (
            ...
            for (
                ...
                for (
                        ...
                        m < 2; m++) {
                    r1[j * 2 + l + 4 * m] =
                            vzip_u16(vreinterpret_u16_u8(r0[j + 4 * m].val[l]),
                                     vreinterpret_u16_u8(r0[j + 2 + 4 * m].val[l])
                            );
                }
            }
        }
...
/* Load and interleave groups of 8 bytes (64 bytes) to the structure r0 */
        for (
                ...
                j < 4; j++) {
            r0[j] =
                    vzip_u8(vld1_u8(src + (2 * j) * total_elements + k * 8),
                            vld1_u8(src + (2 * j + 1) * total_elements + k * 8)
                    );
        }
/* Interleave 16 bytes */
        for (
        ...
        for (
            ...
            for (
                ...
                for (
                        ...
                        m < 2; m++) {
                    r2[j * 2 + l + 4 * m] =
                            vzip_u32(vreinterpret_u32_u16(r1[j + 4 * m].val[l]),
                                     vreinterpret_u32_u16(r1[j + 2 + 4 * m].val[l])
                            );
                }
            }
        }
