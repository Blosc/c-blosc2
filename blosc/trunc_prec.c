...
void truncate_precision32(uint16_t filters_meta, int nelems,
                          int32_t* src, int32_t* dest) {
  int zeroed_bits = BITS_MANTISSA_FLOAT - filters_meta;
...
void truncate_precision64(uint16_t filters_meta, int nelems,
                          int64_t* src, int64_t* dest) {
  int zeroed_bits = BITS_MANTISSA_DOUBLE - filters_meta;
...
void truncate_precision(uint16_t filters_meta, int32_t typesize,
  ...
  switch (typesize) {
    case 4:
      return truncate_precision32(filters_meta, nbytes / typesize,
                                  (int32_t *)src, (int32_t *)dest);
    case 8:
      return truncate_precision64(filters_meta, nbytes / typesize,
                                  (int64_t *)src, (int64_t *)dest);
  }
}
