...
int main(void) {
  ...
  blosc2_storage storage = {.sequential=true, .urlpath="umeta.b2frame", .cparams=&cparams, .dparams=&dparams};
  blosc2_schunk* schunk = blosc2_schunk_new(storage);


  // Add a metalayer
  int umlen = blosc2_add_metalayer(schunk, "umetalayer", (uint8_t *) "This is a umetalayers content...", 10);
  if (umlen < 0) {
  ...
  umlen = blosc2_add_umetalayer(schunk, "umetalayer", (uint8_t *) "This is a umetalayers content...", 32);
  if (umlen < 0) {
    printf("Cannot write umetalayers chunk");
    return umlen;
  }
  ...
  // Add some umetalayers data
  umlen = blosc2_add_umetalayer(schunk, "umetalayer2", (uint8_t *) "This is a content...", 10);
  if (umlen < 0) {
    printf("Cannot write umetalayers chunk");
    return umlen;
  }

  umlen = blosc2_update_umetalayer(schunk, "umetalayer", (uint8_t *) "This is a another umetalayer content...", 20);
  if (umlen < 0) {
    printf("Cannot write umetalayers chunk");
    return umlen;
  }
