...
int main() {
  ...
  int64_t cframe_len;
  bool needs_free;
  BLOSC_ERROR(caterva_to_cframe(arr, &cframe, &cframe_len, &needs_free));

  caterva_array_t *dest;
  BLOSC_ERROR(caterva_from_cframe(cframe, cframe_len, true, &dest));

  /* Fill dest array with caterva_array_t data */
  uint8_t *data_dest = malloc(size);
  BLOSC_ERROR(caterva_to_buffer(dest, data_dest, size));
