...
int main(void) {
    ...
    for (nchunk = 0; nchunk < NCHUNKS - 2; nchunk++) {
    ...
    //Trying to append two small chunks
    nchunk = nchunk - 2;
    static int32_t last_data[CHUNKSIZE - 200];
    static int32_t last_data_dest[CHUNKSIZE - 200];
    for(nchunk = nchunk - 2; nchunk < NCHUNKS; nchunk++) {
        for (i = 0; i < CHUNKSIZE - 200; i++) {
            last_data[i] = i * nchunk;
        }
        int nchunks = blosc2_schunk_append_buffer(schunk, last_data, isize);
        if (nchunks != nchunk + 1) {
            printf("Unexpected nchunks!");
            return -1;
        }
